...
SELECT
    ...
    TO_CHAR(lpc.visit_date, 'WW') AS weeks,
    CASE
        WHEN lpc.visitors_count = 0 THEN 0
        ELSE ROUND(c.total_cost / lpc.visitors_count, 2)
    END AS cpu,
    CASE
        WHEN lpc.leads_count = 0 THEN 0
        ELSE ROUND(c.total_cost / lpc.leads_count, 2)
    END AS cpl,
    CASE
        WHEN lpc.purchases_count = 0 THEN 0
        ELSE ROUND(c.total_cost / lpc.purchases_count, 2)
    END AS cppu,
    CASE
        WHEN c.total_cost = 0 THEN 0
        ELSE ROUND((lpc.revenue - c.total_cost) * 100.00 / c.total_cost, 2)
    END AS roi
FROM last_paid_click AS lpc
LEFT JOIN cost_ad AS c
    ON
